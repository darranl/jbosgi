 ...
 */
package org.jboss.test.osgi.services.urlhandler;
...
public class URLHandlerTestCase {
    ...
    public void testURLHandlerService() throws Exception {
        ...
        try {
            assertEquals("ih//:raboof",
                new String(suckStream(new URL("foobar://hi").openStream())));
        } finally {
            reg.unregister();
        }
    ...
    private static void pumpStream(InputStream is, OutputStream os)
            throws IOException {
    ...
    public static class TestURLConnection extends URLConnection {
        ...
        public InputStream getInputStream() throws IOException {
            InputStream is = new ByteArrayInputStream(new StringBuilder(
                    getURL().toString()).reverse().toString().getBytes());
            return is;
        }
    }
}
